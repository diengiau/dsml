...
## Extension 01: Split training and test sample

Let's do a practice as follows:

1. Run a full model between `medv` and 12 predictors from `Boston` data. Use only the first 450 observations.
2. After having the model, summarize and read the fit model. Check the RSE of the model.
3. Use the model to predict the house price for the rest 56 observations. Check the RSE among the new predicted data (56 obs).
